...
public class StartUpServlet extends HttpServlet {
	...
	try {
		...
		if (currentUser != null) {
			...
			dao.searchProductForAdmin(1);
			int numberOfPage = dao.calculatePageAdmin();
	    ...
	    if (!isAdmin) {
		TblProductDAO dao = new TblProductDAO();
		dao.searchProductActive(1);
		int numberOfPage = dao.calculatePageActive();
		List<TblProductDTO> listProductActive = dao.getListProductActive();
		if (listProductActive != null) {
		    request.setAttribute("LIST_PRODUCT", listProductActive);
		    request.setAttribute("CURRENT_PAGE", 1);
		    request.setAttribute("NUMBER_OF_PAGE", numberOfPage);
		}
	    }
