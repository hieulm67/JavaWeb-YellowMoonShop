...
import hieulm.tblorder.TblOrderDTO;
import hieulm.tblorderdetail.TblOrderDetailDAO;
import hieulm.tblorderdetail.TblOrderDetailDTO;
...
public class TrackOrderServlet extends HttpServlet {
    ...
    static Logger log = Logger.getLogger(TrackOrderServlet.class.getName());

    final String TRACK_ORDER_PAGE = "trackOrderPage";
	...
	try {
		...
		if (session != null) {
			...
			TblOrderDTO orderDTO = orderDAO.searchOrder(orderId, currentUser.getUserEmail());

			if (orderDTO != null) {
			    TblOrderDetailDAO orderDetailDAO = new TblOrderDetailDAO();
			    orderDetailDAO.searchByOderId(orderId);
			    List<TblOrderDetailDTO> listItems = orderDetailDAO.getListItems();

			    if (listItems != null) {

				request.setAttribute("ORDER_ITEMS", listItems);
				request.setAttribute("ORDER_FOUNDED", orderDTO);
			    }
			}
		    }
		}
